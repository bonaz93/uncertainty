...
def GUI():
    ...
    while already_exists:
        ...
        if mode == 'Practice':
            dataframe_name = wdir + '\\data\\part_n_' + PN + '\\Practice_implicit' + PN + '.csv'
            file_name = wdir + '\\data\\part_n_' + PN + '\\Practice_response_implicit' + PN + '.csv'
        else:
            dataframe_name = wdir + '\\data\\part_n_' + PN + '\\Dataframe_implicit' + PN + '.csv'
            file_name = wdir + '\\data\\part_n_' + PN + '\\Response_implicit' + PN + '.csv'
...
def Instructions(part_number, part_name, win, item, mode, device, KeyResp, ori_targ, keyboard, quiT, allowed_keys,\
                 clock_items, esc_keys, dutch):
...
#    \n- Focus in het begin van elke trial op het centrale fixatiekruis en begin dan te zoeken van daaruit.'
 ...
 Er zal altijd een target aanwezig zijn en je moet reageren op de oriëntatie van deze target:\
    ...
    def Wait_and_respond(message, device,keyboard,quiT, allowed_keys,message2 = None, wait = 8,\
                         time_search = 9000, clock_items = clockUseless):
        time.sleep(wait)
        ...
        Press_key, Time_key, ReleaseResponse_loc = Response(device = device, keyboard = keyboard, quiT = quiT,\
                                                                time_search = 9000, allowed_keys = allowed_keys,\
...
#                            alignVert = 'bottom')
    ...
    recap = vis.TextStim(win, text = 'DEFAULT',\
                        color='black', bold = False,\
                        wrapWidth = wrap,\
                        height = 1,\
                        pos=(0,0),\
    ...
    recap.text = recap_message.format(lab[0], lab[1]) if counterbalance else recap_message.format(lab[1], lab[0])
    ...
    if mode == 'Practice':
        ...
        Wait_and_respond(message = textbox_5, device = device, keyboard = keyboard, quiT = quiT,\
                         allowed_keys = allowed_keys, message2 = textbox_6, wait = 6)
...
def breaK(win,device, keyboard, quiT, allowed_keys, clock_items,row, response, rt,dutch, time_search = 500000,\
...
def TrueColors(target_position, target_color, colors):
    '''Should return the right color combination [0,1,2] or [1,2,0] or [2,0,1] to draw the Cshape'''
    color_arr = np.split(np.arange(12), len(colors))
    index = [target_position in i for i in color_arr].index(True)

    true_colors = colors
    while not target_color == true_colors[index]:
        true_colors = np.roll(true_colors, 1)
    return true_colors


def WrongAnswer(keyboard, question, answer, warning, win, device, quiT, rowI, keyss):
    ...
    if keyboard:
        while answer not in keyss:
            ...
            warning.draw()
            win.flip()  
            answer = Response(device = device, keyboard = keyboard, quiT = quiT,\
