...
cue_validity_2, headers, keyboard, control_same_color, row_check, practice, KeyResp,\
participants
...
##POSITION SHOULD BE RANDOM, COLOR NO.
...
for part_num in participants:
    ...
    if practice:
        df_name = wdir + '\\data\\part_n_' + str(part_num) + '\\Practice_implicit' + str(part_num) + '.csv'
    else:
        df_name = wdir + '\\data\\part_n_' + str(part_num) + '\\Dataframe_implicit' + str(part_num) + '.csv'
...
#   number of trials will go on infinitely.
    ...
    for rowI in range(trials_total):
        ...
        weights_distractor_colors = [(1-cue_validity_def[rowI])/(len(colors) - 1)] * (len(colors) - 1)

        # Now append the cue_validity to the other colors' weight
        
        final_weights = np.insert(weights_distractor_colors, cue_identity_def[rowI], cue_validity_def[rowI])
        ...
        # THIS MUST BE CHANGED, COLOR OF THE TARGET MUST BE RANDOM AS THE POSITION.
        color_target[rowI] = np.random.choice(colors, p = final_weights)

        #colors[0] if position_target[rowI] < alfa else (colors[2] if position_target[rowI] >= (number_positions-alfa) else colors[1])
...
#        color_cues[cue_identity_def[rowI]] = np.random.choice(colors, p = weight_predictive_color)
...
#        color_cue2[rowI] = color_cues[2]
        ...
        if color_target[rowI] == cue_identity_def[rowI]:
            valid_trial[rowI] = 1
        else:
            valid_trial[rowI] = 0
    ...
    matrix = np.transpose(np.vstack((position_target, color_target, ori_target,
                        ...
                        if not control_same_color and distractor == distr_ind[-1]:
                            ...
                            if distractor == distr_ind[-1]:  # Perform the check only while filling the last distractor.
                                ...
                                for rand_row in chunk:
                                    ...
                                    if matrix[rand_row,2] == matrix[rand_row,3] == matrix[rand_row,4]:
                                        bool_vect.append(1)
                                        break
                                    else:
                                        bool_vect.append(0)
...
#    ################################################################
    ...
    if not practice:
        for i in set(matrix[:,-5]):
            ...
            frequency_id = round(len(matrix_id[matrix_id[:,1]== i])/len(matrix_id)*100,2)
            print('\nFrequency of color %s where cue_identity is %s is %s%%'%(i,i,frequency_id))
...
#    c1_id0,c2_id0,tot_id0,c0_id1,c2_id1,tot_id1,c0_id2,c1_id2,tot_id2,cue_0_mean,\
#    cue_1_mean,cue_2_mean, color_target_mean, diff_3_cues_percentage, diff_2_cues_percentage,\
